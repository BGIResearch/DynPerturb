...
def get_data_new(dataset_name, different_new_nodes_between_val_and_test=False, randomize_features=False):
    ### 加载数据和特征
    ...
    # 获取所有离散时间戳的唯一值，并排序
    unique_timestamps = sorted(graph_df['ts'].unique())

    # 手动划分时间戳
    # 比如前60%用于训练，接下来20%用于验证，最后20%用于测试
    train_time = unique_timestamps[:int(0.6 * len(unique_timestamps))]  # 前60%的时间戳
    val_time = unique_timestamps[int(0.6 * len(unique_timestamps)):int(0.8 * len(unique_timestamps))]  # 中间20%的时间戳
    test_time = unique_timestamps[int(0.8 * len(unique_timestamps)):]  # 最后20%的时间戳

    # 将sources, destinations, timestamps等从DataFrame中提取
    sources = graph_df.u.values
    destinations = graph_df.i.values
    edge_idxs = graph_df.idx.values
    labels = graph_df.label.values
    ...
    # 根据时间戳划分数据集
    train_mask = graph_df['ts'].isin(train_time)
    val_mask = graph_df['ts'].isin(val_time)
    test_mask = graph_df['ts'].isin(test_time)
    ...
    test_node_set = set(sources[timestamps > min(val_time)]).union(set(destinations[timestamps > min(val_time)]))
    new_test_node_set = set(random.sample(test_node_set, int(0.1 * n_total_unique_nodes)))
