...
class MLP_label(torch.nn.Module):
  def __init__(self, input_dim, num_classes=None, drop=0.3):
    super(MLP_label, self).__init__()
    self.num_classes = num_classes
    self.input_dim = input_dim

    # 每个类别一个独立的 MLP 输出层，不带 sigmoid
    self.mlps = torch.nn.ModuleList([
        torch.nn.Sequential(
            ...
            torch.nn.ReLU(),
            torch.nn.Dropout(drop),
            torch.nn.Linear(10, 1)  # 输出 raw logits
        )
...
class EarlyStopMonitor(object):
  ...
  def early_stop_check(self, curr_val_edge, curr_val_node):
    ...
    elif (curr_val_edge - self.last_best_edge) / np.abs(self.last_best_edge) > self.tolerance and \
          (curr_val_node - self.last_best_node) / np.abs(self.last_best_node) > self.tolerance:
