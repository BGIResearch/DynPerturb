...
parser.add_argument("--memory_updater", type=str, default="gru", choices=["gru", "rnn", "lstm"], help="Memory updater type")
parser.add_argument("--aggregator", type=str, default="last", help="Message aggregator type")
parser.add_argument("--memory_update_at_end", action="store_true", default=True, help="Update memory at the end of batch")
...
parser.add_argument("--use_validation", action="store_true", default=True, help="Use validation set")
...
for i in range(args.n_runs):
    ...
    for epoch in range(args.n_epoch):
        ...
        for batch in train_loader:
            ...
            if node_classification_logits is not None:
                node_classification_loss = torch.nn.CrossEntropyLoss(weight=weights)(node_classification_logits, labels_batch)
            else:
        ...
        logging.info(f"Training Loss: {np.mean(m_loss):.4f} (Epoch Time: {time.time() - start_epoch:.2f}s)")
    ...
    best_model_epoch_tensor = torch.tensor(best_model_epoch if dist.get_rank() == 0 else -1, device=device)
    ...
    try:
        ...
        pred_prob_safe = pred_prob if "pred_prob" in locals() else np.zeros((len(y_true), num_classes))
