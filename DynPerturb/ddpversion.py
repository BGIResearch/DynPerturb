...
from utils.data_processing_mulit import compute_time_statistics, get_data_node_classification, get_data_mulit
...
os.chdir("/home/share/huadjyin/home/s_qinhua2/02code/tgn-master/human_bone/")
...
for i in range(args.n_runs):
    ...
    if rank == 0:
        wandb.init(project='TGN_HumanBone')
    ...
    optimizer = torch.optim.Adam(tgn.parameters(), lr=args.lr, weight_decay=1e-5)
    ...
    for epoch in range(args.n_epoch):
        ...
        if rank==0:
            wandb.log({"train_loss": train_losses[-1], "epoch": epoch + 1})
    ...
    if rank == 0:
        ...
        wandb.log({"Confusion Matrix (Normalized)": wandb.Image(plt.gcf())})
        plt.close()
        ...
        wandb.log({"AUC Bar Chart": wandb.Image(plt.gcf())})
        plt.close()
