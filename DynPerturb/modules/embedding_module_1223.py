...
class GraphEmbedding(EmbeddingModule):
    def __init__(self, node_features, edge_features, memory, neighbor_finder, time_encoder, n_layers,
                 n_node_features, n_edge_features, n_time_features, embedding_dimension, device,
                 n_heads=2, dropout=0.1, use_memory=True, node_id_map=None):
    ...
    def compute_embedding(self, memory, source_nodes, timestamps, celltypes, n_layers, n_neighbors=20, time_diffs=None,
                         use_time_proj=True):
        """Recursive implementation of curr_layers temporal graph attention layers."""
        
        assert (n_layers >= 0)

        print(f"Max node index: {max(source_nodes)}, Min node index: {min(source_nodes)}")
        ...
        for node_id, ts, celltype in zip(source_nodes, timestamps_torch.squeeze(), celltypes):
            #print(f"node_id: {node_id}, ts: {timestamps}, celltype: {celltype}")
            
            # 使用 get() 方法获取特征，找不到时返回 None
            feature = self.node_features.get((node_id.item(), ts.item(), celltype), None)
            
            if feature is None:
                continue  # 跳过该节点
            
        # 将 feature 转换为 Tensor 并加入列表
            source_node_features.append(torch.tensor(feature))
        print(f"Type of timestamps before conversion: {type(timestamps)}") 

        # 将 source_node_features 从列表转换为一个 Tensor
        source_node_features = torch.stack(source_node_features)

        

        if self.use_memory:
            device = memory.device
            source_nodes = source_nodes.to(device)
            source_node_features = source_node_features.to(device)  # 确保 source_node_features 在同一个设备上
        ...
        if n_layers == 0:
            ...
            #neighbors_torch = torch.from_numpy([self.node_id_map[node_id] for node_id in neighbors.flatten()]).long().to(self.device)
            neighbors_torch = torch.from_numpy(neighbors.flatten()).long().to(self.device)
