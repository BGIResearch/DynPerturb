...
class GraphEmbedding(EmbeddingModule):
  ...
  def compute_embedding(self, memory, source_nodes, timestamps, n_layers, n_neighbors=20, time_diffs=None,
    ...
    #source_node_features = self.load_node_features(source_nodes_torch, timestamps_torch)
    #source_node_features = self.node_features[source_nodes_torch, :]
    
     # 加载初始节点特征
    source_node_features = []
    for node, ts in zip(source_nodes, timestamps):
        feature = self.node_features.get((node, ts), np.zeros(self.feature_dim))
        source_node_features.append(torch.tensor(feature, device=self.device))
    ...
    if n_layers == 0:
      ...
      # 获取 edge_features 的最大索引
      max_idx = self.edge_features.size(0)

      # 检查 edge_idxs 是否有超出最大值的情况
      if torch.any(edge_idxs >= max_idx):
          print(f"Invalid edge_idxs found: {edge_idxs[edge_idxs >= max_idx]}")
          print(f"Max index allowed: {max_idx - 1}")
          raise ValueError(f"edge_idxs contains out-of-bound indices. Max index allowed: {max_idx - 1}")
