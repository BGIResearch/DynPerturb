...
class GraphEmbedding(EmbeddingModule):
    ...
    def compute_embedding(self, memory, source_nodes, timestamps, node_features,
        ...
        # 这里必须使用 tensor 类型，因为后续操作需要 GPU 运算
        source_nodes_torch = torch.from_numpy(source_nodes).long().to(self.device)
        timestamps_torch = torch.unsqueeze(torch.from_numpy(timestamps).float().to(self.device), dim=1)

        # 获取当前时间步的节点特征（输入的 node_features 即当前节点时间特征）
        source_node_features = node_features[:len(source_nodes)]
        ...
        neighbor_features = node_features[len(source_nodes):]

        neighbor_embeddings = self.compute_embedding(
            memory,
            neighbors.flatten(),
            np.repeat(timestamps, n_neighbors),
            neighbor_features,  # 递归输入邻居特征
            n_layers=n_layers - 1,
            n_neighbors=n_neighbors
        )
        neighbor_embeddings = self.compute_embedding(
            ...
            neighbors.flatten(),
            np.repeat(timestamps, n_neighbors),
            neighbor_node_features,  # ✅ 保持 node_features 继续传下去
            n_layers=n_layers - 1,
            n_neighbors=n_neighbors
        )
