...
from utils.utils import MergeLayer, MLP_label
from modules.memory_mulit import Memory
from modules.message_aggregator import get_message_aggregator
from modules.message_function import get_message_function
from modules.memory_updater import get_memory_updater
from modules.embedding_module_0208 import get_embedding_module
...
class TGN(torch.nn.Module):
  def __init__(self, neighbor_finder, node_features, edge_features, device, n_layers=2,
    ...
    self.node_classification_decoder = MLP_label(memory_dimension, num_classes, dropout)
  ...
  # def compute_loss(self, pos_score, neg_score, node_classification_prob, labels_batch, link_pred_weight=1.0, node_class_weight=1.0):
  #   """
  #   计算加权后的总损失。
    
  #   参数：
  #   - pos_score: 链接预测任务的正样本分数
  #   - neg_score: 链接预测任务的负样本分数
  #   - node_classification_prob: 多标签分类任务的标签概率
  #   - labels_batch: 标签的真实值，维度为 [batch_size, num_classes]
  #   - link_pred_weight: 链接预测任务的权重
  #   - node_class_weight: 多标签分类任务的权重
    
  #   返回：
  #   - total_loss: 总损失
  #   """
  #   # 1. 链接预测任务的损失
  #   pos_labels = torch.ones(len(pos_score), dtype=torch.float, device=pos_score.device)
  #   neg_labels = torch.zeros(len(neg_score), dtype=torch.float, device=neg_score.device)
  #   link_pred_loss = torch.nn.BCELoss()(pos_score, pos_labels) + torch.nn.BCELoss()(neg_score, neg_labels)

  #   # 2. 多标签分类任务的损失
  #   node_classification_loss = torch.nn.BCEWithLogitsLoss()(node_classification_prob, labels_batch)

  #   # 3. 加权损失
  #   total_loss = link_pred_weight * link_pred_loss + node_class_weight * node_classification_loss
  #   return total_loss
  ...
  def forward(self, source_nodes, destination_nodes, negative_nodes, edge_times,
      ...
      pos_score = score[:n_samples].sigmoid()  # 链接预测的概率
      neg_score = score[n_samples:].sigmoid()  # 链接预测的负样本概率

      # 2. 多标签任务 - 每个节点的多个标签概率
      logits = self.node_classification_decoder(src_emb)  # 每个标签的得分
      # node_classification_prob = torch.sigmoid(logits)  # 每个标签的独立概率
      return pos_score, neg_score, logits

      # return pos_score, neg_score, node_classification_prob
  ...
  #   pos_score = score[:n_samples]  # logits
  #   neg_score = score[n_samples:]  # logits

  #   # 2. 多标签分类 logits
  #   logits = self.node_classification_decoder(src_emb)

  #   return pos_score, neg_score, logits  # 全是 logits，方便外部 loss 处理
